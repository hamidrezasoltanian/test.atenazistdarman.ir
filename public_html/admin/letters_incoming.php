<?php
/*
 * فایل: public_html/admin/letters_incoming.php
 * توضیحات: مدیریت نامه‌های وارده (نمایش دقیق گیرندگان از جدول واسط، رفع مشکل اسکرول، فیلتر شمسی، رسپانسیو و اکسل)
 */
...
if (function_exists('hasPermission') && !hasPermission('letters_incoming') && $_SESSION['role'] !== 'admin') {
    die('<div style="text-align:center; padding:50px; font-family:Vazirmatn, Tahoma; color:red; font-weight:bold;">⛔ شما مجوز مشاهده نامه‌های وارده را ندارید.</div>');
}

$userId = (int)$_SESSION['user_id'];
$isAdmin = (in_array($_SESSION['role'] ?? '', ['admin', 'manager']));

$pageTitle = '📥 نامه‌های وارده';
$letterType = 'incoming';
$basePath = '../';
...
$searchSender = $_GET['search_sender'] ?? '';
...
// --- ساخت کوئری داینامیک ---
$where = " WHERE l.type = 'incoming' AND (l.is_archived = 0 OR l.is_archived IS NULL) AND (l.is_deleted = 0 OR l.is_deleted IS NULL) ";
$params = [];

// امنیت: محدودیت مشاهده نامه‌های وارده برای غیر ادمین
if (!$isAdmin) {
    $where .= " AND (
        l.created_by = :uid1 
        OR l.id IN (SELECT letter_id FROM letter_receivers WHERE receiver_type = 'user' AND receiver_id = :uid2)
...
if ($searchSender) {
    $where .= " AND l.sender_external_name LIKE :sender ";
    $params[':sender'] = "%$searchSender%";
}
if ($dateFrom) {
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    header('Content-Disposition: attachment; filename=Incoming_Letters_'.date('Y-m-d').'.csv');
    $output = fopen('php://output', 'w');
    fprintf($output, chr(0xEF).chr(0xBB).chr(0xBF)); // UTF-8 BOM
    fputcsv($output, ['شماره اندیکاتور', 'موضوع', 'فرستنده (خارجی)', 'گیرندگان (داخلی)', 'تاریخ ثبت (شمسی)', 'وضعیت']);
    
    // کوئری مخصوص با واکشی گروهی گیرندگان
    $stmtEx = $pdo->prepare("SELECT l.*, 
                            (SELECT GROUP_CONCAT(CONCAT(u.first_name, ' ', u.last_name) SEPARATOR '، ') 
                             FROM letter_receivers lr 
                             JOIN users u ON lr.receiver_id = u.id 
...
// کوئری واکشی اطلاعات همراه با لیست دقیق گیرندگان
$dataSql = "SELECT l.id, l.subject, l.indicator_number, l.status, l.sender_external_name, l.registered_at, l.created_at,
                   (SELECT GROUP_CONCAT(CONCAT(u.first_name, ' ', u.last_name) SEPARATOR '، ') 
                    FROM letter_receivers lr 
                    JOIN users u ON lr.receiver_id = u.id 
                    WHERE lr.letter_id = l.id AND lr.receiver_type = 'user') as receivers_names
...
<style>
    * { box-sizing: border-box; font-family: "Vazirmatn", Tahoma, sans-serif !important; }
    ...
    /* قفل کردن عرض جدول بدون اسکرول افقی روی دسکتاپ */
    .l-table { width: 100%; border-collapse: collapse; background: #fff; min-width: 700px; table-layout: fixed; }
    ...
    .col-ind { width: 12%; }
    .col-subj { width: 28%; line-height: 1.6; }
    .col-sender { width: 14%; }
    .col-receiver { width: 16%; }
    .col-date { width: 10%; }
    .col-status { width: 8%; }
    ...
    .btn-refer { background: #f3e8ff; color: #4338ca; border: 1px solid #c7d2fe; }
    ...
    @media print {
        ...
        .col-subj { width: 33% !important; text-align: right !important; }
        .col-sender { width: 18% !important; }
        .col-receiver { width: 12% !important; }
...
<main class="main-content" id="mainContent">
    <div class="content-wrapper">
        <div class="page-header d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div><span class="page-title fw-bold" style="font-size:1.2rem;">📥 نامه‌های وارده</span></div>
        ...
        <div class="filter-card">
            <form method="GET">
                <div class="filter-wrapper">
                    ...
                    <div class="filter-item">
                        <label class="form-label small fw-bold">فرستنده (خارجی)</label>
                        <input type="text" name="search_sender" class="form-control" value="<?php echo htmlspecialchars($searchSender); ?>" placeholder="نام شخص یا شرکت...">
                    </div>
                    <div class="filter-item">
        ...
        <div class="card" style="padding: 20px; background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
                ...
                <table class="l-table">
                    <thead>
                        <tr>
                            ...
                            <th class="col-sender">فرستنده (خارجی)</th>
                            <th class="col-receiver">گیرنده (داخلی)</th>
                    ...
                    <tbody>
                        <?php if(count($letters) > 0): ?>
                            ...
                            <tr>
                                ...
                                <td class="fw-bold" style="color:#2563eb;" data-label="موضوع:"><?php echo htmlspecialchars($l['subject']); ?></td>
                                <td data-label="فرستنده:"><?php echo htmlspecialchars($l['sender_external_name'] ?: '---'); ?></td>
                                <td data-label="گیرنده:"><?php echo htmlspecialchars($l['receivers_names'] ?: '---'); ?></td>
                                <td data-label="تاریخ:">
                                ...
                                <td class="no-print" data-label="عملیات:">
                                    <div class="btn-group-row">
                                        ...
                                        <?php else: ?>
                                            ...
                                            <a href="letter_refer.php?id=<?php echo $l['id']; ?>" class="action-btn btn-refer">📤 ارجاع</a>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
            ...
            <div class="pagination no-print">
                <?php for($i=1; $i<=$totalPages; $i++): ?>
                    <a href="?page=<?php echo $i; ?>&search_subject=<?php echo urlencode($searchSubject); ?>&search_indicator=<?php echo urlencode($searchIndicator); ?>&search_sender=<?php echo urlencode($searchSender); ?>&date_from_fa=<?php echo urlencode($dateFromFa); ?>&date_to_fa=<?php echo urlencode($dateToFa); ?>" 
