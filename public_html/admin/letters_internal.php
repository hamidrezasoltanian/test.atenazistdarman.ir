<?php
/*
 * فایل: public_html/admin/letters_internal.php
 * توضیحات: مدیریت نامه‌های داخلی (فیلتر شمسی، رسپانسیو، قفل عرض ستون‌ها با table-layout: fixed، چاپ و اکسل)
 */
...
if (function_exists('hasPermission') && !hasPermission('letters_internal') && $_SESSION['role'] !== 'admin') {
    die('<div style="text-align:center; padding:50px; font-family:Vazirmatn, Tahoma; color:red; font-weight:bold;">⛔ شما مجوز مشاهده نامه‌های داخلی را ندارید.</div>');
}

$userId = (int)$_SESSION['user_id'];
$isAdmin = (in_array($_SESSION['role'] ?? '', ['admin', 'manager']));

$pageTitle = '🏢 نامه‌های داخلی';
$letterType = 'internal';
$basePath = '../';
...
// --- ساخت کوئری داینامیک ---
$where = " WHERE l.type = 'internal' AND (l.is_archived = 0 OR l.is_archived IS NULL) AND (l.is_deleted = 0 OR l.is_deleted IS NULL) ";
$params = [];

// امنیت: محدودیت مشاهده نامه‌های داخلی برای غیر ادمین
if (!$isAdmin) {
    $where .= " AND (
        l.created_by = :uid1 
...
if (isset($_GET['export']) && $_GET['export'] === 'excel') {
    ...
    header('Content-Disposition: attachment; filename=Internal_Letters_'.date('Y-m-d').'.csv');
    $output = fopen('php://output', 'w');
    fprintf($output, chr(0xEF).chr(0xBB).chr(0xBF)); // UTF-8 BOM
    fputcsv($output, ['ایجاد کننده', 'شماره اندیکاتور', 'موضوع', 'تاریخ ثبت (شمسی)', 'وضعیت']);
    
    $stmtEx = $pdo->prepare("SELECT l.*, u.first_name, u.last_name FROM letters l LEFT JOIN users u ON l.created_by = u.id $where ORDER BY l.created_at DESC");
...
$dataSql = "SELECT l.*, u.first_name, u.last_name 
            FROM letters l 
...
<style>
    * { box-sizing: border-box; font-family: "Vazirmatn", Tahoma, sans-serif !important; }
    ...
    .l-table { width: 100%; border-collapse: collapse; background: #fff; min-width: 850px; table-layout: fixed; }
    ...
    .col-ind { width: 15%; }
    .col-subj { width: 35%; line-height: 1.6; }
    .col-date { width: 15%; }
    .col-status { width: 8%; }
    ...
    @media print {
        ...
        .col-subj { width: 45% !important; text-align: right !important; }
...
<main class="main-content" id="mainContent">
    <div class="content-wrapper">
        <div class="page-header d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <div><span class="page-title fw-bold" style="font-size:1.2rem;">🏢 نامه‌های داخلی</span></div>
        ...
        <div class="card" style="padding: 20px; background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.02);">
                ...
                <table class="l-table">
                    ...
                    <tbody>
                        <?php if(count($letters) > 0): ?>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr><td colspan="6" style="padding:40px; color:#94a3b8; text-align:center; font-weight:bold;">هیچ نامه‌ای یافت نشد.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <div class="pagination no-print">
                <?php for($i=1; $i<=$totalPages; $i++): ?>
                    <a href="?page=<?php echo $i; ?>&search_subject=<?php echo urlencode($searchSubject); ?>&search_indicator=<?php echo urlencode($searchIndicator); ?>&date_from_fa=<?php echo urlencode($dateFromFa); ?>&date_to_fa=<?php echo urlencode($dateToFa); ?>" 
