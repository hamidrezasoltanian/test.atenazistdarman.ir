<?php
/*
 * فایل: public_html/admin/letter_create.php
 * توضیحات: ایجاد نامه جدید با صدور شماره قطعی غیرتکراری و ارجاع خودکار سیستمی به گیرندگان
 */
...
if (function_exists('hasPermission') && !hasPermission('letters_create') && $_SESSION['role'] !== 'admin') {
    die('<div style="text-align:center; padding:50px; font-family:Vazirmatn, Tahoma; color:red; font-weight:bold;">⛔ شما مجوز ایجاد نامه را ندارید.</div>');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_action'])) {
    ...
    if (empty($subject)) {
        $msg = "وارد کردن موضوع نامه الزامی است.";
        $msgType = "error";
        ...
        try {
            ...
            $cols = ['type', 'subject', 'status', 'priority', 'classification', 'department_prefix', 'indicator_number', 'registered_at', 'content', 'created_by', 'fiscal_year_id', 'created_at', 'sender_external_name'];
            $vals = [$type, $subject, $status, $priority, $classification, $departmentPrefix, $indicatorNumber, $registeredAt, $content, $userId, $fiscalYearId, $senderExternalName];
            $placeholders = ['?', '?', '?', '?', '?', '?', '?', '?', '?', '?', '?', 'NOW()', '?'];

            if (function_exists('db_has_column')) {
                if (db_has_column($pdo, 'letters', 'is_archived')) { $cols[] = 'is_archived'; $vals[] = 0; $placeholders[] = '?'; }
                if (db_has_column($pdo, 'letters', 'is_deleted')) { $cols[] = 'is_deleted'; $vals[] = 0; $placeholders[] = '?'; }
            }

            $colStr = implode(', ', $cols);
            $placeholderStr = implode(', ', $placeholders);
            
            $stmt = $pdo->prepare("INSERT INTO letters ($colStr) VALUES ($placeholderStr)");
            $stmt->execute($vals);
            ...
            // ---- اضافه شده: ارجاع خودکار برای نامه‌های داخلی و وارده به گیرندگان ----
            ...
            if (function_exists('logSystem')) logSystem('Letters', 'create', $letterId, "ثبت نامه با موضوع: $subject");

            if ($action === 'pending' && function_exists('send_user_notification')) {
                send_user_notification($pdo, $userId, "📄 نامه ذخیره شد", "نامه «{$subject}» در دست اقدام قرار گرفت.", "admin/letters_pending.php", 'letter', $letterId);
            }

            $pdo->commit();

            if ($action === 'draft') header("Location: letters_drafts.php?msg=draft_saved");
            elseif ($action === 'pending') header("Location: letters_pending.php?msg=letter_created");
            exit;
...
<main class="main-content" id="mainContent">
    <div class="content-wrapper">
        <div class="page-header page-actions d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <div><span class="page-title fw-bold" style="font-size:1.2rem;">📝 ایجاد نامه جدید</span></div>
            <div><a href="letters_pending.php" class="btn btn-secondary btn-sm px-3 fw-bold">بازگشت به کارتابل من</a></div>
        </div>
            ...
            <form method="POST" enctype="multipart/form-data" id="letterForm">
                ...
                <div class="form-group" style="margin-bottom: 20px;">
                    ...
                    <div class="radio-group">
                        <label class="radio-card active" id="lbl-internal">
                            <input type="radio" name="type" value="internal" checked onchange="toggleFields()"> 
                            <i class="fas fa-building"></i><span>داخلی</span>
                        </label>
                        <label class="radio-card" id="lbl-outgoing">
                            <input type="radio" name="type" value="outgoing" onchange="toggleFields()"> 
                            <i class="fas fa-paper-plane"></i><span>صادره</span>
                        </label>
                        <label class="radio-card" id="lbl-incoming">
                            <input type="radio" name="type" value="incoming" onchange="toggleFields()"> 
                ...
                <div class="form-row">
                    <div class="form-col" style="flex: 2;">
                        ...
                        <input type="text" name="subject" class="form-control" required placeholder="موضوع نامه...">
                    </div>
                    <div class="form-col">
                    ...
                    <div class="form-col">
                        ...
                        <select name="department_prefix" class="form-control">
                            ...
                            <option value="ب">ب (بازرگانی)</option>
                            <option value="ف">ف (فروش)</option>
                            <option value="م‌ع">م‌ع (مدیرعامل)</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="form-row">
                    <div class="form-col hidden-field" id="box-sender-ext">
                        ...
                        <input type="text" name="sender_external_name" id="sender_external_name" class="form-control border-danger" placeholder="مثال: شرکت آتنازیست درمان...">
                    </div>
                    ...
                    <div class="form-col" id="box-receiver-int">
                        ...
                        <div id="int-receivers-container" class="chip-container">
                            <div class="text-muted small w-100 text-center" id="int-empty-text" style="line-height:25px;">لیست گیرندگان خالی است.</div>
                        </div>
                    </div>
                    ...
                    <div class="form-col hidden-field" id="box-receiver-ext">
                        ...
                        <div id="ext-receivers-container" class="chip-container">
                            <div class="text-muted small w-100 text-center" id="ext-empty-text" style="line-height:25px;">لیست مشتریان خالی است.</div>
                        </div>
                    </div>
                    ...
                    <div class="form-col hidden-field" id="box-signers">
                        ...
                        <div id="signers-container" class="chip-container">
                            <div class="text-muted small w-100 text-center" id="signer-empty-text" style="line-height:25px;">امضاکننده‌ای انتخاب نشده.</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="attachment-wrapper">
                    <label class="form-label fw-bold mb-3"><i class="fas fa-paperclip"></i> مستندات و پیوست‌ها (اختیاری)</label>
                    <div class="attachment-header">
                        ...
                        <span class="text-muted small">پس از انتخاب، فایل‌ها به لیست پایین اضافه می‌شوند.</span>
                    </div>
                    <input type="file" id="tempFileInput" style="display:none;" multiple accept=".pdf,.doc,.docx,.jpg,.png,.zip,.rar" onchange="attachFiles()">
                    <div class="file-list" id="fileListUI"></div>
                    <input type="file" name="attachments[]" id="realFileInput" multiple style="display:none;">
                </div>

                <div class="action-buttons">
                    <button type="submit" name="submit_action" value="draft" class="btn btn-outline-secondary btn-action-wrapper" onclick="return validateForm()">💾 ذخیره پیش‌نویس</button>
